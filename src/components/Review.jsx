   ...
   <section className='container mx-auto mb-8 mt-12' id="review">
    ...
    className='flex flex-col'>
        ...
        className='flex item-center justify-center gap-6'>
            ...
            <h6>{REVIEW.name}</h6>
            <p className='text-5m text-neutral-500'>{REVIEW.profession}</p>
            </div>
        </motion.div>
    </motion.div>
    <motion.div 
    ...
    className='mt-14 flex flex-col items-center justify-center gap-2 md:flex-row'>
        {[customer1, customer2, customer3, customer4].map((customer, index) => (
            <motion.img
            variants={ItemVariants}
             key={index} src={customer} alt="customer" className='h-[300px] w-[200px] rounded-br-3xl rouned-tl-3xl object-cover'/>
        ))}
    </motion.div>
   </section>
  )
